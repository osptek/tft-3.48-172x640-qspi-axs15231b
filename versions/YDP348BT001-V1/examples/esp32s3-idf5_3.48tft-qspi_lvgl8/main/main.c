...
#define EXAMPLE_PIN_NUM_LCD_TE            (GPIO_NUM_NC)  //代码里面未使用
...
#define EXAMPLE_TOUCH_INT           (GPIO_NUM_40)
...
static void example_lvgl_touch_cb(lv_indev_drv_t *drv, lv_indev_data_t *lv_data)
{
    ...
    /* 3. 反馈给 LVGL（丢弃越界噪点，避免偶发大值干扰） */
    if (err == ESP_OK && tp_cnt > 0 &&
        point.x < EXAMPLE_LCD_H_RES && point.y < EXAMPLE_LCD_V_RES) {
...
void app_main(void)
{
...
#if (EXAMPLE_TOUCH_INT !=-1)
    /* TP INT */
    gpio_config_t tp_int_gpio_config = {
        .mode = GPIO_MODE_INPUT,
        .pin_bit_mask = 1ULL << EXAMPLE_TOUCH_INT,
        .pull_up_en = GPIO_PULLUP_ENABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE
    };
    ESP_ERROR_CHECK(gpio_config(&tp_int_gpio_config));
#endif
   ...
   const spi_bus_config_t buscfg = AXS15231B_PANEL_BUS_QSPI_CONFIG(EXAMPLE_PIN_NUM_LCD_PCLK,
    ...
    /* esp_lcd_touch 先 mirror 再 swap：x_max/y_max 必须按交换前的触摸原始坐标系配置。
     * 本屏显示为 172x640，触摸原始轴为 X=长边(640)、Y=短边(172)，再 swap 对齐显示。 */
